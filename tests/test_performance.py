...
def test_search_for_apartment_large_dataset():
    ...
    existing_apartment_key = "apart-80901"
    non_existent_apartment_key = "apart-1000000"
    
    ok_search_time = time.perf_counter()
    result_ok = manager.get_apartment(existing_apartment_key)
    ok_search_time = (time.perf_counter() - ok_search_time) * 1e3
    fail_search_time = time.perf_counter()
    result_fail = manager.get_apartment(non_existent_apartment_key)
    fail_search_time = (time.perf_counter() - fail_search_time) * 1e3
    assert isinstance(result_ok, Apartment)
    assert result_fail is None
    assert ok_search_time< ALLOWED_SEARCH_TIME_MS, (
        f"Searching for existing apartment in {N} apartments took "
        f"{ok_search_time:.3f}ms, limit {ALLOWED_SEARCH_TIME_MS}ms"
    )
    assert fail_search_time <ALLOWED_SEARCH_TIME_MS, (
        f"Searching for non-existing apartment in {N} apartments took "
        f"{fail_search_time:.3f}ms, limit {ALLOWED_SEARCH_TIME_MS}ms"
    )